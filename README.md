...
## 🖥 Веб-интерфейсы (Фронтенд)

В проект включены два легковесных веб-интерфейса (работают без сторонних зависимостей на встроенном сервере Node.js):

* 👑 **Панель Администратора (Admin / HR Portal):** [http://localhost:3000](http://localhost:3000)
  * Управление компаниями, филиалами, должностями и сотрудниками.
  * Формирование графиков и смен, назначение персонала и просмотр журнала аудита изменений.
  * Согласование заявок на отпуск и табель явок (бесконечная прокрутка `Slice`).
* 👤 **Личный Кабинет Сотрудника (Employee Workspace):** [http://localhost:3001](http://localhost:3001)
  * Персональное расписание и назначенные смены.
  * Интерактивная отметка явок: приход на смену (**Check-In**) и завершение смены (**Check-Out**).
  * Подача заявок на отпуск, отгул или больничный лист с отслеживанием статуса и резолюции руководителя.
...
```
*(Сервер автоматически поднимет порт `3000` для админки и порт `3001` для кабинета сотрудника).*
